...
namespace RolandK.Utils.Collections;
...
public class RingBuffer<T> : IEnumerable<T>
{
    ...
    {
        var actStart = _itemStart;
        var actSize = _itemLength;
        for (var loop = 0; loop < _itemLength; loop++)
        {
            yield return _buffer[(actStart + loop) % _buffer.Length];
        }
    }
