...
using RolandK.Checking;
using RolandK.Patterns.BackgroundLoops;
...
public class FirLibMessenger : IFirLibMessagePublisher, IFirLibMessageSubscriber
{
    ...
    public static FirLibMessenger GetByName(string messengerName)
    {
        ...
        if (result == null) { throw new CommonLibException($"Unable to find Messenger for thread {messengerName}!"); }
        return result;
    }
    ...
    public void ConnectToGlobalMessaging(FirLibMessengerThreadingBehavior checkBehavior, string messengerName, SynchronizationContext? hostSyncContext)
    {
        ...
        {
            throw new CommonLibException($"This messenger is already registered as '{_globalMessengerName}'!");
        }
        if (s_messengersByName.ContainsKey(messengerName))
        {
            throw new CommonLibException($"The name '{messengerName}' is already in use by another messenger!");
        }
        ...
        TMessageType message, bool isInitialCall)
    {
        ...
        try
        {
            ...
            if (isInitialCall)
            {
                ...
                var asyncTargets = s_messagesAsyncTargets.GetOrAdd(currentType, (_) => FirLibMessageHelper.GetAsyncRoutingTargetMessengersOfMessageType(currentType));
