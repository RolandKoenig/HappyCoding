...
namespace HappyCoding.EFIncludePerformance
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            await TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync(dbOptions, random, true);
            ...
            // Check include with OrderBy and Take(1) with additional Where
            await TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync(dbOptions, random);
            await TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync(dbOptions, random);
            await TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync(dbOptions, random);
            await TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync(dbOptions, random);
            ...
            bool discardOutput = false)
        {
            ...
            if (!discardOutput)
            {
                ...
                var millisecondsPerRun = totalMilliseconds / COUNT_RUNS_PER_EXPERIMENT;
                Console.WriteLine(
                    $"{nameof(TestRandomGetByIncludeWithOrderByAndTakeOptimizedAsync)}: " +
                    $"{totalMilliseconds:N2} ms total, " +
                    $"{millisecondsPerRun:N2} ms per run");
            }
        }
