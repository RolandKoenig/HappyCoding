...
public class FirLibMessenger : IFirLibMessagePublisher, IFirLibMessageSubscriber
{
    ...
    {
        messengerName.MustNotBeNullOrEmpty(nameof(messengerName));

        var result = TryGetByName(messengerName);
        if (result == null) { throw new MessengerException($"Unable to find Messenger for thread {messengerName}!"); }
        return result;
    }
    ...
    public void ConnectToGlobalMessaging(FirLibMessengerThreadingBehavior checkBehavior, string messengerName, SynchronizationContext? hostSyncContext)
    {
        ...
        {
            throw new MessengerException($"This messenger is already registered as '{_globalMessengerName}'!");
        }
        if (s_messengersByName.ContainsKey(messengerName))
        {
            throw new MessengerException($"The name '{messengerName}' is already in use by another messenger!");
        }
    ...
    {
        actionOnMessage.MustNotBeNull(nameof(actionOnMessage));
    ...
    {
        actionOnMessage.MustNotBeNull(nameof(actionOnMessage));
        messageType.MustNotBeNull(nameof(messageType));
    ...
    {
        messageSubscription.MustNotBeNull(nameof(messageSubscription));
